...
MAX_EPOCHS = 2000  # Arbitrary number of test cycles. Default 1000!
...
class NSchwefel:
        ...
        while not done:
            ...
            for i in range(popSize):
                if i != minimum:
                    ...
                    if math.fabs(thisChromo.get_fitness()) < math.fabs(thatChromo.get_fitness()):
                        minimum = i
    ...
    def exchange_mutation(self, index, exchanges):
        ...
        self.mutations += 1
        """
        sys.stdout.write("Vorher: "+str(thisChromo.toStr()+"\n"))
        sys.stdout.write("Exchange Mutation verwendet.\n")
...
#
    ...
    def partially_mapped_crossover(self, chromA, chromB, child1, child2):
        ...
        sys.stdout.write(" hat "+str(thatChromo.get_fitness())+" fitness\n")
        thatChromo.toStr()
    ...
    def prep_next_epoch(self):
        ...
        for i in range(popSize):
            while len(self.population) > START_SIZE:
                #worst=self.population[self.get_maximum()]
                #sys.stdout.write(str(self.population[self.get_maximum()].get_fitness()) + " fitness DES SCHELCHTESTEN\n")
    ...
    def print_best_solution(self, bestSolution=ChromosomeSchwefel(MAX_LENGTH,type)):
        board = []
        for i in range(self.mMaxLength):
...
if __name__ == '__main__':
    ...
    while(counter != 1):
        ...
        #nSch.show_permutation_amount()
        nSch.show_crossover_per_epoche()

        array = np.array(array) + nSch.get_best_crossover()
        counter += 1
